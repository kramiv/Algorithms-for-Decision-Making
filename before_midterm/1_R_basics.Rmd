...
```{r}
madrigal.names[madrigal.haspower == TRUE]
...
```{r}
typeof(mirabel.lst[[2]][[1]])#  integer
typeof(mirabel.lst[[3]][[1]])#  double
typeof(mirabel.lst[[5]][2])#  character     extracts contents
#$
...
madrigal.lst <- list (name = madrigal.names,
                      ...
                      height = madrigal.heights,
                      has.power = madrigal.haspower )
marginal.lst <- as_tibble(madrigal.lst)
marginal.lst
```
...
```{r}
as_tibble(madrigal.tbl$height)
as_tibble(madrigal.df$height)
...
str(madrigal.df)
```
tibble converts type of each column to be the same type?
but, in a data frame, the types can be different

6. What happens if you try to create a `data.frame` or a `tibble` with two columns of different sizes?  

```{r}
madrigal.df2 <- data.frame(name = c("name1", "name2", "name3"),
                      age = c(1,2))
madrigal.df2


madrigal.tbl2 <- tibble(name = c("name1", "name2", "name3"),
                      age = c(1,2))
madrigal.tbl2
```
does not work:
dataframe: "arguments imply differing number of rows: 3, 2"
...
```{r}
madrigal.levels <- c("first","second","third")# form levels
madrigal.generation <- factor (c("first","third","second","third","third"),
                               levels=madrigal.levels)# assign levels
madrigal.generation
...
```{r}
typeof(madrigal.df$name)# integer
typeof(madrigal.tbl$name)#  character
