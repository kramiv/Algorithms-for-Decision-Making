...
classification: response is categorial. binary or more levels. 
prediction: response is continuous numerical
...
```
'if majority of digits around me are 5, then I classify as 5'
huh. so it's not a mean? do even numbers work?
...
notes:
'how do I avoid looking at my testing dataset for an optimum value of k'?
cross-validation
divide training dataset into a fold. select, for example, fold = 5. (k = 5)
divide training dataset into 5 parts. 
use one of the folds as the testing dataset
other 4 folds are training. 
then calculate 5 MSE values. and take average. that is average MSE value. 
...
******
note solutions are slightly different

can also do 
training(digits.fold$splits[2][[1]])
testing(digits.fold$splits[2][[1]])

```{r}
original.folds <- digits.folds %>%
  vfold_cv(train.127.tbl, v = 10)
typeof(original.folds)
...
#digits.test <- testing(digits.folds[[1]])#requires edited out stuff under set.seed
#digits.train <- training(digits.folds[[1]])#requires edited out stuff under set.seed
training(digits.folds$splits[2][[1]])
testing(digits.folds$splits[2][[1]])
```
digits.folds <- initial_split(Auto, strata = NULL, prop = .1)
why not use initial split? when is this used?
...
tune_res <- tune_grid(
  object = knn.wf,
  resamples = digits.folds, #tablemate said removing edited out stuff from udner set_seed made this work
  grid = neighbors.tbl
)

autoplot(tune_res)
```
best k=value is at about 47?
...
best.neighbor <- select_best(tune_res, metric = "accuracy")
...
when k = 40, misclassification is about 23 or 24%
Note accuracy graph.

also keep in mind these numbers are based on cross-validation for the training dataset. 
haven't done anything with testing yet.
...
```{r}
augment(knn.final.fit, test.127.tbl) %>%
  conf_mat(truth = y, estimate = .pred_class)

augment(knn.final.fit, test.127.tbl) %>%
  accuracy(truth = y, estimate = .pred_class)

plot_boundary(knn.final.fit, test.127.tbl, 0.01)#this parameter is how fine the grid is.
```
cross-validation: numbers are a little inflated. testing dataset accuracy is a little lower. 
...
note: this is prediction, not classification

```{r}
#Training/Testing dataset
set.seed(12345)
initial_split(mn.police.tbl, prop = 0.5)
test.police.tbl <- testing(initial_split(mn.police.tbl, prop = 0.5))
train.police.tbl <- training(initial_split(mn.police.tbl, prop = 0.5))
...
recipe <- recipe(tot ~ week, data=train.police.tbl)
...
recipe <- recipe(tot ~ week, data=train.127.tbl)
