...
```{r}
a.tbl <- recipe.raw.tbl %>%
  group_by(Recipe_Index) %>%
  summarize(Recipe_Index = Recipe_Index,
            Rating = Rating,
            Ingredient = Ingredient,
            Quantity = mean(Quantity))
recipe.wide.tbl <- a.tbl %>%
  pivot_wider(names_from = Ingredient, values_from = Quantity, values_fn = mean)

recipe.wide.tbl
```
Not quite ordered alphabetically.

I need to fix something here. so tired. 
...
Removes rows without a rating. 
Selects recipe index with the rating and 4 major ingredients
Formats the name of some of the major ingredients.
removes recipes without the 4 major ingredients selected.

Basically selects for recipes with a rating and the 4 major ingredients.
...
recipe.final.tbl <- recipe.filter.tbl %>%
  group_by(Recipe_Index) %>%
  summarize(flour = flour, 
            chocolate = chocolate, 
            butter = butter, 
            sugar = sugar, 
            total = sum(flour+chocolate+butter+sugar),
...
```{r}
recipe.final.tbl %>%
  pivot_longer(cols = prop.flour:prop.sugar, 
               names_to = "props",
               values_to = "propnums") %>%
  group_by(props) %>%
  ggplot() +
    geom_boxplot(mapping = aes(props, propnums))

```
The graph is right but the values from earlier are wrong.
...
Input = proportion of flour, chocolate, butter.
Response variable = Rating.

This is a prediction problem. (predict a number - not necessarily predict over some period of time)
...
```{r}
set.seed(54321)
digits.folds <- vfold_cv(recipe.train.tbl, v = 10)
digits.folds

neighbors.tbl <-  tibble(neighbors=seq(5,30, by=5))
neighbors.grid.tbl <- grid_regular(neighbors(range = c(5, 30)), 
                                   levels = 6)
neighbors.grid.tbl
