...
augment(lda.fit, test.127.tbl) %>%
  pivot_longer(cols = .pred_1:.pred_7, names_to = "name", values_to = "value") %>%
...
confused: 1 and 2 (132); 2 and 7 (99) (biggest numbers not on diagonal)
note: better results on extremes. 2 is stuck in between the two numbers. makes sense it is part of both pairs of most confused numbers.

accuracy: the sum of the diagonal in confusion matrix / total values
(proportion of correct guesses in classification)
```{r}
(300+144+288)/(300+132+15+48+144+99+78+93+288)
augment(lda.fit, test.127.tbl) %>%
  pivot_longer(cols = .pred_1:.pred_7, names_to = "name", values_to = "value") %>%
  accuracy(truth = y, estimate = .pred_class)
```

3. Create an lda model that uses both $x_1$ and $x_2$ as input variables and calculate the accuracy of your method. Is this an improvement over 1)? Create the confusion matrix using our dataset and check what pairs of digits are getting misclassified the most. 
```{r}
lda.model.3 <- discrim_linear() %>%
  set_engine("MASS") %>%
  set_mode("classification")

recipe.3 <- recipe(y ~ x_1+x_2, data=train.127.tbl)

lda.wflow.3 <- workflow() %>%
  add_recipe(recipe.3) %>%
  add_model(lda.model.3) 

lda.fit.3 <- fit(lda.wflow.3, train.127.tbl)
lda.fit.3
```

```{r}
augment(lda.fit.3, test.127.tbl) %>%
  pivot_longer(cols = .pred_1:.pred_7, names_to = "name", values_to = "value") %>%
  conf_mat(truth = y, estimate = .pred_class)
```
          Truth
...
1 and 2 confused: 123
2 and 7 confused: 96.
These numbers are just slightly lower than the previous confusion matrix's maximum confused values.
...
```{r}
grid.vec.x_1 <- seq(from = 0, to = 1, by = .01)
grid.vec.x_1
grid.vec.x_2 <- seq(from = 0, to = 1, by = .01)
grid.tbl <- expand_grid(x_1 = grid.vec.x_1, x_2 = grid.vec.x_2)
grid.tbl
```

```{r}
pred <- augment(lda.fit.3, grid.tbl) %>%
  pivot_longer(cols = .pred_1:.pred_7, names_to = "name", values_to = "value")
pred %>%
...
```{r}
lda.model.5 <- discrim_quad() %>%
  set_engine("MASS") %>%
  set_mode("classification")

recipe.5 <- recipe(y ~ x_1+x_2, data=train.127.tbl)

lda.wflow.5 <- workflow() %>%
  add_recipe(recipe.5) %>%
  add_model(lda.model.5) 

lda.fit.5 <- fit(lda.wflow.5, train.127.tbl)
lda.fit.5
```

```{r}
augment(lda.fit.5, test.127.tbl) %>%
  pivot_longer(cols = .pred_1:.pred_7, names_to = "name", values_to = "value") %>%
  conf_mat(truth = y, estimate = .pred_class)
```
          Truth
...
It is much better. It might fit better because 1 is so skewed. 
...
```{r}
pred <- augment(lda.fit.5, grid.tbl) %>%
  pivot_longer(cols = .pred_1:.pred_7, names_to = "name", values_to = "value")
pred %>%
  ggplot(aes(x_1, x_2, fill = .pred_class)) +
    geom_raster()
...
The areas for each predicted value are much rounder. 
previous model: creates lines that separates the different classes. 
this model: variance is not assumed to be the same across all classes. quadratic lines are used to separate points. 



# No homework for next week, but remember your challenge is due next Wednesday 3/16!
********
what was teh thing with bayes and k-near? 
note solution key: it uses functions.

wait how did he do the points on the graph thing?

maybe reformat the project and do the fancy plots thing. 
