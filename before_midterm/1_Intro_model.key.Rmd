...
```{r echo=FALSE}
pred.knn <- predict(knn.model, mn.police.tbl)
...
(mse.test <- mean ((train.mn.police.tbl$tot-pred.train)^2))

pred.test<- predict(knn.model, test.mn.police.tbl)
(mse.test <- mean ((test.mn.police.tbl$tot-pred.test)^2))
```
...
plot_knn(7,mn.police.tbl)
plot_knn(14,mn.police.tbl)
plot_knn(35,mn.police.tbl)
plot_knn(70,mn.police.tbl)
plot_knn(140,mn.police.tbl)
...
calc_MSE_knn <- function(kNear, tbl){
  ...
  pred.train<- predict(knn.model, train.tbl)
  mse.train <- mean ((train.tbl$tot-pred.train)^2)

  pred.test<- predict(knn.model, test.tbl)
  mse.test <- mean ((test.tbl$tot-pred.test)^2)
...
```{r echo=FALSE}
kNear=50
knn.model <- knnreg(tot~week+wday, data=train.mn.police.tbl,k=kNear)
