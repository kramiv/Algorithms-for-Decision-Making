...
good image of bias vs. variance:
https://www.cs.cornell.edu/courses/cs4780/2018fa/lectures/images/bias_variance/bullseye.png

you want a mix of both to maximize a model that fits the data and is accurate.

bias/variance tradeoff with linear regression, ridge regression. 

say you want to fit a linear model on 3 points. n = 3.
Use simulated data which is where you know what the truth is. x1, one input variable. And y, the output variable. assume x1 comes from normal distribution (mean = 0, SD = 1). pick points from x-axis at random using a random distribution. 

x1~N(0,1)

Next, construct y: y = b*x . Very simple linear model. 
Then, add a little bit of error because real life has a little bit of error. 

y = b*x + error

and assume error is distributed N(0,sigma)

let b = 1. The slope is y = x then. And sigma = 2. (why these values? is this standard?)
...
ggplot(sim.tbl.2, aes(x1,y)) +
      ...
      xlim(-6,6)+
      ylim(-6,6)+
      geom_smooth(method=lm, formula = y~0+x,#forcing an intercept of 0. linear regression formula. the only parameter is the slope
                  se=FALSE, fullrange=TRUE)
```
...
map_drf: if x = (x1, ..., xn), then it applies the 2nd argument as a function to the first argument, a set of numbers. 
...
In this plot, about 11 of the plots out of 20 look correct.

Let's also point out that if the number of points $N$ increases we get a more uniform set of models

note: less variation in this version. The first one with fewer data = high variation. 
...
sim.recipe <- 
  recipe(formula = y~x1, data = sim.tbl)

lm.model <- linear_reg() %>%
  set_engine("lm")

lm.wflow <- workflow() %>%
  add_recipe(sim.recipe) %>%
  add_model(lm.model) 

lm.fit <- fit(lm.wflow, sim.tbl)
...
3 data points, 100 simulations

```{r}
set.seed(54321)
N <- 3
no2.tbl <- map_dfr(1:100, calc_slope_lm)
```
...
set.seed(55057)
N <- 10
no3.tbl <- map_dfr(1:100, calc_slope_lm)
...
```{r}
N <- 3
no3.tbl <- map_dfr(1:20, calc_coefs, ridge.model)
```
```{r}
#pivot_longer: names_to, values_to. Or just (x1:x2) works.
no3.tbl %>%
  pivot_longer(x1:x2, names_to = "xval") %>%
  mutate(xval = as.factor(xval)) %>%
  ggplot(aes(xval, value)) +
  geom_boxplot()
```
```{r}
#trying to get mean and sd
no3.tbl %>%
  pivot_longer(x1:x2, names_to = "xval") %>%
  group_by(xval) %>%
  mutate(mean = mean(xval))
```


note: some answers on pdf

mean of linear model is very close to 1, so bias is very small. 

mean of ridge model is 0.5. More bias. Width is slightly less. Note lambda = 1. 

ridge model: lambda/penalty in formula intead of just linear model introduces more bias but less variance. 
bias shrinks coefficients and makes them smaller. MSE is a combination of the bias and variance. 
