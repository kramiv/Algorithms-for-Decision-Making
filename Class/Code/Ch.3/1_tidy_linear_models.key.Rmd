...
ggplot(ames.train, aes(x=log_Sale_Price))+
  geom_histogram()

ggplot(ames.test, aes(x=log_Sale_Price))+
  geom_histogram()
```

## Creating linear models with `parsnip`

We are interested in predicting the `log_Sale_Price` as a linear of `Longitude` and `Latitude`. In order to do that we will be using `parsnip` which provides a standarized interface to create models. 

The first part consist of declaring the type of model we will be using (linear regression using `linear_reg`) and what implementation (or engine) of linear regression we will be using (`lm`). We can do do this usign the following code:
...
Notice that this set of steps corresponds to the second step of our class, namely fitting the model using our training dataset. For completeness let's include all of the steps in a single chunk:
...
```{r}
rmse(ames.test, log_Sale_Price, .pred)
rmse_vec(ames.test$log_Sale_Price, ames.test$.pred)^2
```


## Summary

Below we provide the summary of how to use `tidymodels` on the Ames dataset to predict `log_Sale_Price` based on `Latitude` and `Longitude`
...
# Summarize information from the linear model
tidy(lm.fit)
glance(lm.fit)

# Adding the prediction to the testing dataset and calculate MSE
ames.test <- lm.fit %>%
  augment(new_data = ames.test)
rmse_vec(ames.test$log_Sale_Price, ames.test$.pred)^2
