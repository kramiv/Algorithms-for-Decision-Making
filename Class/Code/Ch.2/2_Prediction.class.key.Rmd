...
calc_MSE <- function(kNear, train.tbl, test.tbl) {
    ...
    pred <- predict(knn.model, test.tbl) 
    mean ((pred-test.tbl$tot)^2)
}
...
```{r echo=FALSE}
mse.test <- vector(length = 52) # create an empty vector with length=52
for(i in 1:52){
  mse.test[i] <- calc_MSE(7*i, train.mn.police.tbl, test.mn.police.tbl)
}
```
...
```{r echo=FALSE}
mse.train <- vector(length = 52) # create an empty vector with length=365
for(i in 1:52){
  mse.train[i] <- calc_MSE(7*i, train.mn.police.tbl, train.mn.police.tbl)
}
...
mse.tbl %>%
  ggplot(aes(x=k, y=MSE, color=Dataset)) +
  geom_line()
