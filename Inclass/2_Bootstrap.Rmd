...
```{r}
qda.predict <- predict(qda.fit, test.127.tbl)
...
bootstrap.tbl <- bootstraps(train.127.tbl, 50)

bootstrap.tbl$splits[[10]]
bootstrap.tbl$splits[[10]] %>% analysis() # bootstrap
bootstrap.tbl$splits[[10]] %>% assessment() # out of bag

# out of bag will be a different size because it is based on chance what values are not
# selected by the bootstrapping operation

# the bootstrap datasets will be the same size because we are generating a new dataset
# of the same size as the original using the bootstrapping operation
...
calc_correct_twos_qda <- function(split) {
  ...
  recipe <- recipe(y~x_1+x_2, data=train)
  
  qda.wflow <- workflow() %>%
    add_recipe(recipe) %>%
    add_model(qda.model)
  
  conf.mat <- augment(qda.fit, test) %>%
    conf_mat(truth = y, estimate = .pred_class) %>%
    tidy()
...
calc_correct_twos_qda(bootstrap.tbl$splits[[8]])
calc_correct_twos_qda(bootstrap.tbl$splits[[3]])
```
