...
```{r}
neighbors.tbl <- tibble(neighbors = seq(1,51,5))
...
mn.police.split.tbl <- initial_split(mn.police.tbl, prop = 0.5)
mn.police.train.tbl <- training(mn.police.split.tbl)
mn.police.test.tbl <- testing(mn.police.split.tbl)

# Model specification

recipe <- recipe(tot ~ week + wday, data = mn.police.train.tbl)
...
digits.folds <- vfold_cv(mn.police.train.tbl, v = 10)
